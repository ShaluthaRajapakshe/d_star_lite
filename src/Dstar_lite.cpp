...
namespace PathPlanners_all
{
	...
	bool PathPlannersROS::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal, std::vector<geometry_msgs::PoseStamped> &plan)
	{
		...
		{ //goal frame and global frame shoudl be the same
	...
	void PathPlannersROS::UpdateVertex(node predecessor, node start, float g_and_rhs[][2], int k_m)
	{
		...
		//update vertex function (Need to check whether predecessor.cell_index is in OPL or not)
		for (uint i = 0; i < OPL.size(); i++)
		{
			if (OPL[i].first.cell_index == predecessor.cell_index)
			{
				if_in_OPL = true;
			}
		}

		// if ((g_and_rhs[predecessor.cell_index][0] != g_and_rhs[predecessor.cell_index][1]) && if_in_OPL)
		// {
		// 	//Update OPL
		// 	for (uint i = 0; i < OPL.size(); i++)
		// 	{
		// 		if (OPL[i].first.cell_index == predecessor.cell_index)
		// 		{
		// 			OPL[i].second = CalculateKey(predecessor, start, g_and_rhs, k_m).second;
		// 		}
		// 	}
		// }
		...
		{
			OPL.push_back(CalculateKey(predecessor, start, g_and_rhs, k_m)); //Insert to OPL
		}
		...
		if (if_in_OPL)
		{
			...
			for (uint i = 0; i < OPL.size(); i++)
			{
				if (OPL[i].first.cell_index == predecessor.cell_index)
				{
					OPL.erase(OPL.begin() + i); //check whether i or i-1
				}
			}
		}
	}
	...
	vector<int> PathPlannersROS::constructPath(int startCell, int goalCell, float g_score[])
	{
		...
		{ //start cell eka enakn passen passata enawa
			vector<int> neighborCells;
